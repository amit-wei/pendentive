...
raises work that names no agent when the user must be told. `DEC-108` sends a
gated unit only when its raiser releases it, at a time that the request may state,
and closes it when the approval lapses; the released unit goes to Actuation, which
carries it to the holder of its act (`REQ-WC-069`). `OPN-048`, `OPN-063`,
...
`OPN-077` in Interaction. `INT-PL-38` waits on `OPN-079` in Authorization. `DEC-105` puts no gated step
